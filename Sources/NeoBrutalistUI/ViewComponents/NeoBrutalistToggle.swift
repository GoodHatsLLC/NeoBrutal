        ...
        .overlay(
          Group {
            if configuration.isOn {
              RoundedRectangle(cornerRadius: trackCornerRadius, style: .continuous)
                .stroke(.nb.accent.primary, lineWidth: nbTheme.borderWidth)
            } else {
              RoundedRectangle(cornerRadius: trackCornerRadius, style: .continuous)
...
extension ToggleStyle where Self == NeoBrutalistToggleStyle {
  public static func neoBrutalist(size: NeoBrutalistToggleStyle.Size = .regular)
    -> NeoBrutalistToggleStyle
  {
